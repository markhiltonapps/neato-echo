...
`;
  ...
  async processAudio(audioBlob, metadata = {}, processingPipeline = null) {
    ...
    try {
      ...
      if (useLocalWhisper) {
        ...
        if (!isSignedIn) {
          const err = new Error(
            "Neato Echo Cloud requires sign-in. Please sign in again or switch to BYOK mode."
          );
  ...
  async startStreamingRecording(forceDefaultMic = false) {
    ...
    try {
      ...
      if (error.name === "NotAllowedError" || error.name === "PermissionDeniedError") {
        ...
        errorDescription =
          "Your Neato Echo Cloud session is unavailable. Please sign in again from Settings.";
  ...
  async _finalizeStreamingRecording(sessionId) {
    ...
    if (!finalText && durationSeconds > 2 && fallbackBlob?.size > 0) {
      ...
      if (target === "skip") {
        logger.warn(
          "Skipping batch fallback: Neato Echo Cloud session signed out",
          {},
          "streaming"
        );
      } else {
